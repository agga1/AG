...
struct Event {
    ...
    explicit Event(int value=0, int from=0, int to=0, bool start=false) {
        this->value = value;
        this->from = from;
        this->to = to;
        this->start = start;
...
int main() {
    ...
    for (int i = 0; i < testCases; i++) {
        bool found = false;
        cin >> V >> E;
        int *connNr = new int[2 * V + 1];
        memset(connNr, 0, (2 * V + 1) * sizeof(int));
        int nrOfZeroes = 2 * V;
        vector<Event> events;
        Graph g(V);
        for (int j = 0; j < E; j++) {
        ...
        while (it < events.size()) {
            ...
            for (auto e:currEvents) {
                if (e.start) {
                    ...
                    connNr[e.from]--;
                    connNr[V + e.to]--;
                    if (connNr[V + e.to] == 0) nrOfZeroes++;
                    if (connNr[e.from] == 0) nrOfZeroes++;
                }
            }
            if (nrOfZeroes == 0 && g.hopcroftKarp() == V) { // max matching is perfect, we found the answer
